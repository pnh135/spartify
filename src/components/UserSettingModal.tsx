...
//     const profileImage= await handleImgUpload() as string;
...
export default function UserSettingModal({
  ...
  const handleImgUpload = useCallback(async () => {
    if (!imageUrl) return;

    const file = imageUrl;
    const imageExt = file.name.split(".").pop();
    let baseName = file.name.replace(`.${imageExt}`, "");
    ...
    const { data, error } = await supabase.storage
      .from("user-image")
      .upload(`public/${baseName}.${imageExt}`, file);
    ...
    const postImgUrl = `https://wbbxeuloxzmatnfryegy.supabase.co/storage/v1/object/public/user-image/${data?.path}`;
    return postImgUrl;
  }, [imageUrl]);
